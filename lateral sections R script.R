...
# Daten für LOf_extended vorbereiten
LOf_extended <- LOf %>%
  mutate(
    mean_plus_sd = mean + sd,
    mean_minus_sd = mean - sd,
    mean_x = as.numeric(factor(Genus)),
    mean_plus_sd_x = as.numeric(factor(Genus)),
    mean_minus_sd_x = as.numeric(factor(Genus))
  )

# Daten für LOm_extended vorbereiten
LOm_extended <- LOm %>%
  mutate(
    mean_plus_sd = mean + sd,
    mean_minus_sd = mean - sd,
...
# Daten für MOm_extended vorbereiten
MOm_extended <- MOm %>%
  mutate(
    mean_plus_sd = mean + sd,
    mean_minus_sd = mean - sd,
...
# Daten für MOf_extended vorbereiten
MOf_extended <- MOf %>%
  mutate(
    mean_plus_sd = mean + sd,
    mean_minus_sd = mean - sd,
...
# Ursprünglicher Plot LOf_g
LOf_g <- ggplot(data = LOf, aes(x = Genus, y = mean, fill = Region)) +
  geom_point(width = 0.2, shape = 21, size = 5, color = "black", position = position_dodge(width = 0.4)) +
  geom_point(data = LOf_extended, aes(x = mean_plus_sd_x, y = mean_plus_sd, fill = Region), 
             shape = 24,  # Dreieck nach oben
             color = "black", 
             size = 2.5, 
             position = position_dodge(width = 0.4)) +
  geom_point(data = LOf_extended, aes(x = mean_minus_sd_x, y = mean_minus_sd, fill = Region), 
             shape = 25,  # Dreieck nach unten
             color = "black", 
             size = 2.5, 
             position = position_dodge(width = 0.4)) +
  theme(
...
LOm_g <- ggplot(data = LOm, aes(x = Genus, y = mean, fill = Region)) +
  geom_point(width = 0.2, shape = 21, size = 5, color = "black", position = position_dodge(width = 0.4)) +
  geom_point(data = LOm_extended, aes(x = mean_plus_sd_x, y = mean_plus_sd, fill = Region), 
             shape = 24,  # Dreieck nach oben
             color = "black", 
             size = 2.5, 
             position = position_dodge(width = 0.4)) +
  geom_point(data = LOm_extended, aes(x = mean_minus_sd_x, y = mean_minus_sd, fill = Region), 
             shape = 25,  # Dreieck nach unten
             color = "black", 
             size = 2.5, 
             position = position_dodge(width = 0.4)) +
...
MOm_g <- ggplot(data = MOm, aes(x = Genus, y = mean, fill = Region)) +
  geom_point(width = 0.2, shape = 21, size = 5, color = "black", position = position_dodge(width = 0.4)) +
  geom_point(data = MOm_extended, aes(x = mean_plus_sd_x, y = mean_plus_sd, fill = Region), 
             shape = 24,  # Dreieck nach oben
             color = "black", 
             size = 2.5, 
             position = position_dodge(width = 0.4)) +
  geom_point(data = MOm_extended, aes(x = mean_minus_sd_x, y = mean_minus_sd, fill = Region), 
             shape = 25,  # Dreieck nach unten
             color = "black", 
             size = 2.5, 
             position = position_dodge(width = 0.4)) +
...
MOf_g <- ggplot(data = MOf, aes(x = Genus, y = mean, fill = Region)) +
  geom_point(width = 0.2, shape = 21, size = 5, color = "black", position = position_dodge(width = 0.4)) +
  geom_point(data = MOf_extended, aes(x = mean_plus_sd_x, y = mean_plus_sd, fill = Region), 
             shape = 24,  # Dreieck nach oben
             color = "black", 
             size = 2.5, 
             position = position_dodge(width = 0.4)) +
  geom_point(data = MOf_extended, aes(x = mean_minus_sd_x, y = mean_minus_sd, fill = Region), 
             shape = 25,  # Dreieck nach unten
             color = "black", 
             size = 2.5, 
             position = position_dodge(width = 0.4)) +
...
####xx####
  geom_smooth(data = do, aes(y = mean), method = "lm", color = "orange", se = FALSE) +
  geom_smooth(data = vt, aes(y = mean), method = "lm", color = "orange", se = FALSE)
  


ggplot(data = vt, aes(x = reorder(Genus, mean), y = mean, shape = Type, colour = Region)) +
...
ggplot(data = Rblength_mean_sd, aes(x = reorder(Genus, mean), y = mean, shape = Type, colour = Region))+
  geom_point()+
  geom_errorbar(aes(ymin = mean - sd, ymax = mean + sd), width = 0.1) 


ggplot(data = Rblength_mean_sd, aes(x = interaction(Sex,Genus), y = mean, shape = Type, colour = Region)) +
  geom_point ()


MO <- filter(Rblength_mean_sd, Type == "MO")
ggplot(data = MO, aes(x = interaction(Sex,reorder(Genus,mean)), y = mean, shape = Type, colour = Region)) +
  geom_point ()
LO <- filter(Rblength_mean_sd, Type == "LO")
ggplot(data = LO, aes(x = interaction(Sex,Genus), y = mean, shape = Type, colour = Region)) +
  geom_point ()
...
male <- filter(Rblength_mean_sd, sex == "m")
ggplot(data = male, aes(x = interaction(genus), y = mean, shape = type, colour = region))+
  geom_point()

female <- filter(Rblength_mean_sd, sex == "f")
ggplot(data = female, aes(x = interaction(genus), y = mean, shape = type, colour = region))+
  geom_point()

#compare to lens surface area?
LT  <- read.table(paste0(wd,"/save/LensSurfaceR2.txt"), header = TRUE, dec =",") 
df_merged <- Rblength_mean_sd %>%
  left_join(LT,select(Genus,Sex,area_l,Type), by = c("Genus", "Sex","Type"))

ggplot(data = df_merged, aes(x = area_l , y = mean, shape = type, colour = Genus))+
  geom_point()

df_merged$ratio_Rhblength <- df_merged$mean/df_merged$area_l

ggplot(data = df_merged, aes(x = reorder(Genus,ratio_Rhblength) , y = ratio_Rhblength, shape = Type, colour = Region))+
  geom_point()

#quantify the difference between do and vt for each genus? 

#work with length master instead?
ggplot(data= length_master, aes(y = Length, x = interaction(Genus,Type) , colour = Region))+
  geom_boxplot()
#divide by Type
MO <- filter(length_master, Type == "MO")

ggplot(data= MO, aes(y = Length, x = interaction(Sex, Genus) , colour = Region))+
  geom_boxplot()


 ggplot(data = MO, aes(x = interaction(Region,Genus), y = Length, colour = Region))+
  
  #geom_flat_violin( color = "black", width = 1.5, position=position_nudge(x=0.15, y=0), alpha = 0.5) +
  
  #geom_boxplot(color = "black", size = 1.2, outlier.shape= NA, alpha=1, width=0.1, position=position_nudge(x=0.15, y=0)) +
  
  geom_point(shape = 16, size = 0.8, color = "black", position=position_jitter(width=.09, height=0), size= 1) 




LO <- filter(length_master, Type == "LO")
ggplot(data= LO, aes(y = Length, x = interaction(Genus,Type) , colour = Region))+
  geom_boxplot()
